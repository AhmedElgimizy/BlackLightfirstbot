...
『#arole <rank> / لأعطاء رتبة للجميع』
『#clear / مسح الشات』
『#mute @user <reason> / اعطاء العضو ميوت لازم رتبة <Muted>』
『#unmute @user / لفك الميوت عن الشخص 』
『#kick @user <reason> / طرد الشخص من السيرفر』
『#ban @user <reason> / حضر الشخص من السيرفر』
『❖!cc / ينشا لك الوان مع كم الوان تبي』
   `,`
          ...
          .then(co => {
            text = co.first().content
              ...
              message.channel.send(`تم حفظ اقتراحك الرجاء انتضار الرد من قبل الاداره`)
                client.channels.get("515574347629592596").send(`${message.author.username}'s sug => ${text}`)
...
client.on('guildMemberAdd', member => { //LAST CODES -HONRAR-
  member.guild.fetchInvites().then(guildInvites => {
    ...
    const stewart = member.guild.channels.find("name", "invites");
...
client.on("message", message => {
   ...
   if(message.author.bot) return;
      ...
      .setTitle(`This Is The Logo Of : ** ${message.guild.name} **`)
...
client.on("message", message => {
  if(message.content.startsWith("#verfry")) { // الامر والبريفكس
...
client.on('message', message => {
  ...
  if(!message.member.hasPermission('ADMINISTRATOR')) return      message.channel.send('ليس لديك البرمشنات الكافية. ADMINISTRATOR' );  //7md
    ...
    reaction1.on("collect", r => {
    message.channel.send(ballot_box_with_check |   ${message.guild.members.size} Sending. | white_check_mark).then(m => m.delete(5000));  //7md
    message.guild.members.forEach(m => {
    var bc = new  
       Discord.RichEmbed()
       .setColor('0f0d0d')
       .addField('» BlackLight Broadcast.', args)  //7md
