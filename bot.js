...
client.on('guildMemberAdd', member => {
     const welcomer =  member.guild.channels.find('blacklight', 'اسم روم الترحيب');
const w = ['./w1.png'];
...
client.on("ready", () => {
    var guild;
    while (!guild)
        guild = client.guilds.get("466721000239661057 ");
...
client.on("guildMemberAdd", (member) => {
    let channel = member.guild.channels.get("466728749845446667");
    if (!channel) {
